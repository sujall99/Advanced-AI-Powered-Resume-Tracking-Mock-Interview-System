...
## Job Role Classifier (Train Your Own) 🧠

This project includes a baseline job-role classifier (TF-IDF + Logistic Regression).

- **Sample dataset**: `data/job_roles_sample.csv` (columns: `text`, `role`)
- **Training script**: `scripts/train_job_role_classifier.py`
- **Saved model path**: `models/job_role_classifier.joblib`

Train (from the project folder):

```bash
python scripts/train_job_role_classifier.py --data data/job_roles_sample.csv --out models/job_role_classifier.joblib
```

After training, the Streamlit app will automatically load the saved model (and fall back to a demo model if missing).
